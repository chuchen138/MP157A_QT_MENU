    ...
    ui(new Ui::MusicPlayer)
{
    ...
    this->setWindowTitle(tr("孙相国音乐播放器"));
...
void MusicPlayer::setVol(int vol)
{
...
{
    QStringList songsList=QFileDialog::getOpenFileNames(this,"选择文件",QDir::homePath(),"*.mp3 *.raw *.wav");
...
void MusicPlayer::playPause()
{
    ...
    if(" "==ui->playPauseButton->text())
    {
        ...
        if(-1!=index)
        {
            ...
            player->play();
            ui->playPauseButton->setText("\n");
            ui->label_background->setStyleSheet("background-image: url(:/pic/MusicPlayerBGPlay.png);");
            //            读取歌词文件
    ...
    else
    {
        player->pause();
        ui->playPauseButton->setText(" ");
        ui->label_background->setStyleSheet("background-image: url(:/pic/MusicPlayerBGStop.png);");
        //    清空歌词
//        ui->label->clear();
//        ui->label_2->clear();
...
void MusicPlayer::durChanged(qint64 dur)
{
...
void MusicPlayer::posChanged(qint64 pos)
{
    presentTime.clear();
...
//根据播放进度的改变设置播放进度
void MusicPlayer::setPos(int pos)
{
    setPlayTime();
}

//清空播放列表
void MusicPlayer::clearList()
{
    playList->clear();
    ui->songsList->clear();
}
//在播放列表中删除某首歌曲
void MusicPlayer::delectSong()
{
    int index=ui->songsList->currentRow();
    if(-1!=index)
    {
...
//设置有音量和静音
void MusicPlayer::volumeSilence()
{
    ...
    {
        ui->volumeButton->setText(" ");
        ui->label_volume->setStyleSheet("background-image: url(:/pic/MusicPlayer_04.png);");
        ui->volumeSlider->setValue(50);
    }
...
void MusicPlayer::on_back_ptn_clicked()
{
    this->close();
    this->deleteLater();
}
